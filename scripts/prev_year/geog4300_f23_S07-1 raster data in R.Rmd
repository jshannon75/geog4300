...
ga_cty_rast<-rasterize(ga_cty,dem,field="FID_1")
plot(ga_cty_rast)

ga_cty_elev<-terra::zonal(rast(dem),rast(ga_cty_rast),"mean",na.rm=TRUE)

ga_cty$meanelev <- ga_cty_elev$min
```
...
```{r}
tm_shape(ga_cty)+
  tm_polygons("meanelev",style="jenks")
```
