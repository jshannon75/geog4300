...
There are several packages available to create and edit spatial data in R. This includes both raster and vector data. This script focuses on the latter. The relatively new sf (stands for simple features) package is one efficient way to load vector data. Other popular packages for spatial data are rgdal, sp, and raster.
...
charpop<-st_read("data/charlotte_tractpop.gpkg") %>%
  ...
  filter(cty_fips==37119) %>% #Select just Mecklenberg Cty
  st_transform(4326) #Transform to WSG84 (crs 4326)
```
