...
Then we'll use group_by and summarise to get the counts of each outcome (0=tails, 1=heads). We can use prop.test to check whether these outcomes were significantly different. Note that prop.test needs the table in matrix format.
...
prop_sample.table<-prop_samplelong %>%
  ...
  summarise(count=n()) %>%
  spread(result,count)
prop_table_matrix<-as.matrix(prop_sample.table) 
prop.test(prop_table_matrix) #Run prop.test on the table
```
...
1) Let's compare liters of coffee sold on campus per 100 students each day. 
```{r}
