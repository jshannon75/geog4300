...
Shapiro-Wilk test: the null hypothesis is that the data IS normal, so a low p value indicates that the dataset is skewed/not noraml.
```{r}
...
The qqline function can also add in a line showing normal distribution
```{r}
...
```
Pick two species and compare their pollen levels using the appropriate t-test.
