...
Easy, right? But the resulting table is hard to read. We can now use 'pivot_wider' to push out the data *wide*. In this case, we just have to identify the variable with the new variable names (using 'names_from') and values (using 'values_from').
...
Now you try it! Calculate the mean value by state for the `intnet_brd_pct` (Percent with broadband internet) and `ugrad_pct` (Percent enrolled in an undergraduate program) variables.
