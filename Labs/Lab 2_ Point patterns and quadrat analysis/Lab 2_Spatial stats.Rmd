...
**Overview:**
This lab covers  two main topics: basic spatial statistics and probability distributions. Your answers to the lab questions should be typed in the response template and turned in using the Assignment Dropbox on the ELC site. 

##Part 1: mapping the data and mean centers##
...
*“SpokanePrecinct_2014_crimecounts” has the number of crimes in 2014 broken down by type for each voting precinct in the city. 
*"SpokanePrecincts_pop" has precinct boundaries and the 2010 population of blocks in those precincts. Note: this is saved as a geojson file, which similar to a shapefile but more compact.
...
spokaneprecinct_2014_crimecounts<-read_csv("https://github.com/jshannon75/geog4300/raw/master/Labs/Lab%202_%20Point%20patterns%20and%20quadrat%20analysis/SpokanePrecinct_2014_crimecounts.csv")

spokaneprecincts<-st_read("https://github.com/jshannon75/geog4300/raw/master/Labs/Lab%202_%20Point%20patterns%20and%20quadrat%20analysis/SpokanePrecincts_pop.geojson")
```

**Question 1 (4 points):** _Using tmap in view mode, create a map using the tmap package that shows the location of all the crimes in the spokane_crimes data frame and the precinct boundaries. Add the Carto Positron basemap as well. See script 6-2 for a model to work from._

**Question 2 (2 points):** _Using the Offense variable and working with the spokane_crimes_all data, select just Murder and Vehicle Theft as crimes. Create a data frame with the mean center of each group. 

**Question 3 (2 points):**_Map the mean centers you created in question 2 using tmap in view mode. To do so, you'll need to use st_as_sf to convert the data frame to sf format. Use tm_dots to show the points, as in this example, which shows all reported crimes for drugs and robbery (not the mean centers, which you need to calculate):
...
**Question 4 (2 points) Interpret how any difference between the mean centers you calculated (e.g., direction and distance) is meaningful. What does it tell you about the locations of these crimes?_
